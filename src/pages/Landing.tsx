  ...
  return (
    <PageShell activeNav="home" transparentHeader>
      ...
      <FeatureSection
        ...
        imagePath="/og-preview-dashboard.svg"
        imageAlt="Dashboard showing Top Pick and Best Window cards"
        reversed
      />
      <FeatureSection
        eyebrow="Offline-first"
        headline="Works in the dead zones."
        body="Bolinas, Sonoma Coast, Highway 1 south of Half Moon Bay — the spots you actually want to surf are the ones with zero cell service. Surf Vikings caches 48 hours of forecast per spot. You open the app from the car at dawn, no bars, and the data is already there."
        imagePath="/og-preview-offline.svg"
        imageAlt="Offline indicator on the dashboard"
        reversed={false}
      />
      <DataProof />
      <FeatureGrid />
...
function FeatureSection({
  eyebrow, headline, body, imagePath, imageAlt, reversed,
}: {
  eyebrow: string; headline: string; body: string;
  imagePath: string; imageAlt: string; reversed: boolean;
}) {
  return (
    <section id="how-it-works" style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <PhoneMockup imagePath={imagePath} imageAlt={imageAlt}/>
        </div>
      </div>
    </section>
  );
...
function PhoneMockup({ imagePath, imageAlt }: { imagePath: string; imageAlt: string }) {
  // Placeholder mockup. The imagePath points at a .svg that doesn't
  // yet exist — renders as a stylized phone with a teal gradient until
  // we swap in real screenshots.
  return (
    <div style={{
      ...
      background: '#0F172A',
      padding: 10,
      boxShadow: '0 40px 80px -20px rgba(15, 23, 42, 0.35), 0 0 0 1px rgba(15, 23, 42, 0.1)',
      position: 'relative',
    }}>
      <div style={{
        ...
        <img src={imagePath} alt={imageAlt} style={{
