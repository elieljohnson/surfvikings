          ...
          <div onClick={() => onOpenSpot('mavericks')} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: TOKENS.textDim, marginTop: 2 }}>18s NW groundswell · Spectator advisory</div>
              </div>
...
function TopPickCard({ spot, timeline, onOpen }: { spot: Spot; timeline: ForecastHour[]; onOpen: () => void }) {
  ...
  return (
    <div onClick={onOpen} style={{
      margin: '16px 20px 8px', background: TOKENS.surface, border: `1px solid ${TOKENS.border}`,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: TOKENS.textDim, marginTop: 2 }}>{spot.subtitle}</div>
        </div>
        ...
        {windows.map((w, i) => (
          <div key={i} onClick={() => onOpen(w.spot.id)} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <span style={{ fontSize: 11, fontWeight: 500 }}>{w.spot.name}</span>
              <span style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 13, fontWeight: 500, color: scoreColor(w.peak) }}>
                {Math.round(w.peak)}
              </span>
            </div>
            <div style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 10, color: TOKENS.textDim, letterSpacing: '0.05em' }}>
