    ...
    <svg width={width} height={height + (showAxis ? 14 : 0)} style={{ display: 'block' }}>
      ...
      {showAxis && [0, 6, 12, 18, 24, 30, 36, 42].map((h) => (
        <text key={h} x={h * barW} y={height + 11}
          fontFamily="JetBrains Mono, ui-monospace, monospace" fontSize="8" fill={TOKENS.textMute}>
...
export function ScoreTimeline({
  ...
  return (
    <svg width={width} height={height + 14} style={{ display: 'block' }}>
      ...
      <text x={width - 2} y={epicY - 2} textAnchor="end" fontFamily="JetBrains Mono, ui-monospace, monospace" fontSize="7" fill={TOKENS.epic} opacity="0.8">EPIC 75</text>
      <text x={width - 2} y={goodY - 2} textAnchor="end" fontFamily="JetBrains Mono, ui-monospace, monospace" fontSize="7" fill={TOKENS.good} opacity="0.65">GOOD 55</text>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: align === 'right' ? 'flex-end' : 'flex-start', gap: 2 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexDirection: align === 'right' ? 'row-reverse' : 'row' }}>
        ...
        <div style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 9, letterSpacing: '0.12em', color: TOKENS.textMute, textTransform: 'uppercase' }}>{label}</div>
      </div>
      ...
      {hint && <div style={{ fontSize: 9, color: TOKENS.textMute }}>{hint}</div>}
    </div>
  );
}
...
export function Rule({ label }: { label?: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '4px 0' }}>
      ...
      <span style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 9, letterSpacing: '0.15em', color: TOKENS.textMute, textTransform: 'uppercase' }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: TOKENS.border }}/>
    </div>
  );
}
      ...
      {tabs.map((t) => {
        ...
        return (
          <button key={t.id} onClick={() => onChange(t.id)} style={{
            ...
            <span style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{t.label}</span>
          </button>
        );
      })}
    </div>
