  ...
  return (
    <Screen>
      <div style={{ padding: '52px 20px 14px', borderBottom: `1px solid ${TOKENS.border}` }}>
        <div style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 9, letterSpacing: '0.18em', color: TOKENS.textMute, textTransform: 'uppercase' }}>Forecast · 48h</div>
      ...
      <div style={{ padding: '0 20px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 8, letterSpacing: '0.12em', textTransform: 'uppercase', color: TOKENS.textDim }}>
...
function HourlyHeatmap({ timeline }: { timeline: ForecastHour[] }) {
  const rows = [
    { label: 'TODAY', data: timeline.slice(0, 24) },
    { label: 'FRI',   data: timeline.slice(24, 48) },
  ];
  return (
    <div style={{ background: TOKENS.surface, border: `1px solid ${TOKENS.border}`, borderRadius: 10, padding: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '36px repeat(24, 1fr)', gap: 1, marginBottom: 4 }}>
        ...
        {[0, 6, 12, 18].map((h) => (
          <div key={h} style={{
            gridColumn: 'span 6',
            fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 8, color: TOKENS.textMute,
            letterSpacing: '0.1em',
      ...
      {rows.map((row, ri) => (
        <div key={row.label} style={{ display: 'grid', gridTemplateColumns: '36px repeat(24, 1fr)', gap: 1, marginBottom: 2 }}>
          <div style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 8, color: TOKENS.textDim, letterSpacing: '0.1em', display: 'flex', alignItems: 'center' }}>{row.label}</div>
          {row.data.map((t, i) => {
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center', fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 8, color: TOKENS.textMute, letterSpacing: '0.1em' }}>
        <span>FLAT</span>
    ...
    <div style={{
      ...
      <div style={{ width: 56 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <div style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 9, letterSpacing: '0.1em', color: TOKENS.textMute, textTransform: 'uppercase' }}>{label}</div>
        </div>
        <div style={{ fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 14, color: TOKENS.text, fontWeight: 500, marginTop: 2 }}>
          {current.toFixed(1)}<span style={{ fontSize: 8, color: TOKENS.textDim, marginLeft: 2 }}>{unit}</span>
        </div>
      </div>
      <div style={{ flex: 1 }}>
        <ForecastChart timeline={timeline} metric={metric} spot={spot} width={220} height={28} showAxis={false}/>
      </div>
      <div style={{ width: 50, textAlign: 'right', fontFamily: 'JetBrains Mono, ui-monospace, monospace', fontSize: 9, color: TOKENS.textMute }}>
